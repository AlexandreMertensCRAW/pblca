...
- `plot_enteric_ch4_by_model.R` — reads all the Monte-Carlo entries
  ...
  `sim_id = "mc_enteric_<variant>"`, generated by section 2bis of
  `run_case_study.py`) and plots a comparison of the model variants:
  one facet per animal group, bars of the central values and p5–p95
  whiskers per variant. Output: `R/fig_ch4_par_modele.png`.
